...
// hard refresh: calling /refresh should always hard refresh tokens
// soft refresh: calling a route that soft-refreshes should always refresh tokens if it is needed (ex: /me)
// no refresh: calling a route with only a check should never refresh tokens (ex: /authorize)
...
func TestRefresh(t *testing.T) {
	...
	t.Run("calling GET /refresh with [invalid access_token, invalid refresh_token] returns 401", func(t *testing.T) {
		...
		t.Run("refresh_token is malformed", func(t *testing.T) {
			...
			err = json.Unmarshal(body, &errorResponse)
			require.NoError(t, err)
			assert.Contains(t, errorResponse["error"], apperrors.ErrNoRefreshToken.Error())
		})
	...
	t.Run("indirect use cases: middleware testing", func(t *testing.T) {
